...
export default function SudokuGrid({
  ...
  // Build the four 2x2 quadrants: top-left, top-right, bottom-left, bottom-right
  const quadrants = [
    { startR: 0, startC: 0 },
    { startR: 0, startC: 2 },
    { startR: 2, startC: 0 },
  ...
  return (
    <div
      className="inline-grid grid-cols-2 gap-[5px] p-3 bg-slate-300 dark:bg-slate-600 rounded-2xl border border-slate-300 dark:border-slate-600 shadow-inner"
      role="grid"
      aria-label="Sudoku grid"
    >
      {quadrants.map(({ startR, startC }) => (
        <div
          key={`q-${startR}-${startC}`}
          className="grid grid-cols-2 gap-1.5 p-1.5 bg-slate-50 dark:bg-slate-900 rounded-xl"
        >
          {[0, 1].map((dr) =>
            [0, 1].map((dc) => {
              const r = startR + dr;
